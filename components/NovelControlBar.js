...
export default function NovelControlBar({ className, style, id, vol, chap }) {
  ...
  return (
    <div className={className} style={style}>
      <a
        onClick={handlePrev}
        className="flex items-center p-4 cursor-pointer group rounded-lg border border-transparent transition-colors hover:bg-neutral-700 mr-28"
      >
      ...
      <select id="chapterSelect" onChange={handleOnChange} className="text-gray-900 text-sm rounded-lg block p-2.5 placeholder-gray-900 text-white">
        {list}
      </select>

      <a
        onClick={handleNext}
        className="flex items-center p-4 cursor-pointer group rounded-lg border border-transparent transition-colors hover:bg-neutral-700 ml-28"
      >
        <h2 className={`text-xl font-semibold`}>
          Sonraki{' '}
          <span className="inline-block transition-transform group-hover:translate-x-1 motion-reduce:transform-none">
