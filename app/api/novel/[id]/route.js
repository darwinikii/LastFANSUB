...
export async function GET(req, { params }) {
  ...
  if (!fs.existsSync(path.join(process.cwd(), "data", params.id, "volumes"))) var volumes = []
  else var volumes = fs.readdirSync(path.join(process.cwd(), "data", params.id, "volumes")).filter(f => !f.startsWith('.'))
